  ...
  Content.find({}, function(err, results){
    ...
    if (results.length === 0){
        ...
        Content.findOne( { contentTitle: "Home" }, function(error, foundItems){
          ...
          res.render("home", { homeContentTitle: foundItems.contentTitle, homeContentBody: foundItems.contentBody , submittedPost: posts });
        });
...
app.get("/posts/:post", function(req, res){
  ...
  posts.forEach(function (element){
    ...
    if(_.lowerCase(req.params.post) === _.lowerCase(element.title)){
